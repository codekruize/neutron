...
import org.neutron.device.j2se.J2SEDeviceButtonsHelper;
import org.neutron.device.j2se.J2SEDeviceDisplay;
import org.neutron.device.j2se.J2SEImmutableImage;
import org.neutron.device.j2se.J2SEInputMethod;
import org.neutron.device.j2se.J2SEMutableImage;
...
public class SwingDeviceComponent extends JPanel implements KeyListener, InputMethodListener, InputMethodRequests {
	...
	public void keyPressed(KeyEvent ev) {
		...
		if (button != null) {
			...
			org.neutron.device.impl.Shape shape = button.getShape();
			if (shape != null) {
	...
	public void keyReleased(KeyEvent ev) {
		...
		pressedButton = null;
		if (prevOverButton != null) {
			org.neutron.device.impl.Shape shape = prevOverButton.getShape();
			if (shape != null) {
				repaint(shape.getBounds());
			}
		}
	...
	protected void paintComponent(Graphics g) {
		...
		if (overButton != null) {
			org.neutron.device.impl.Shape shape = overButton.getShape();
			if (shape != null) {
				drawImageInShape(offg, ((J2SEImmutableImage) device.getOverImage()).getImage(), shape);
			}
		}
		if (pressedButton != null) {
			org.neutron.device.impl.Shape shape = pressedButton.getShape();
			if (shape != null) {
	...
	private void drawImageInShape(Graphics g, Image image, org.neutron.device.impl.Shape shape) {
		...
		if (shape instanceof org.neutron.device.impl.Polygon) {
			Polygon poly = new Polygon(((org.neutron.device.impl.Polygon) shape).xpoints,
					((org.neutron.device.impl.Polygon) shape).ypoints,
					((org.neutron.device.impl.Polygon) shape).npoints);
			g.setClip(poly);
		}
		org.neutron.device.impl.Rectangle r = shape.getBounds();
