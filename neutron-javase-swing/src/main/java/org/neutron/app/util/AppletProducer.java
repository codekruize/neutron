...
import org.neutron.app.classloader.ClassPreprocessor;
import org.neutron.app.classloader.ExtensionsClassLoader;
import org.neutron.app.classloader.InstrumentationConfig;
...
public class AppletProducer {
	...
	public static void createHtml(File htmlOutputFile, DeviceImpl device, String className, File midletOutputFile,
		...
		try {
			...
			writer.write("<html>\n");
			writer.write("\t<head>\n");
			writer.write("\t\t<title>Neutron</title>\n");
			writer.write("\t</head>\n");
			writer.write("\t<body>\n");
			writer.write("\t\t<applet code=\"org.neutron.applet.Main\"\n");
	...
	public static void createMidlet(URL midletInputUrl, File midletOutputFile) throws IOException {
		...
		try {
			...
			URL url = AppletProducer.class.getResource("/neutron-injected.jar");
			if (url != null) {
				ijis = new JarInputStream(url.openStream());
				while ((jarEntry = ijis.getNextJarEntry()) != null) {
					if (jarEntry.getName().equals("org/neutron/Injected.class")) {
				...
				Logger.error("Cannot find neutron-injected.jar resource in classpath");
			}
		} finally {
	...
	private static String usage() {
		return "--midletClass {midlet class name} \n" +
			   "--appletInput {neutron-javase-applet.jar location} \n" +
